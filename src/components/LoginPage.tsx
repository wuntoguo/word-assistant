...
import { registerWithEmail, loginWithEmail, fetchAuthProviders, type AuthProviders } from '../api';
...
export default function LoginPage() {
  ...
  const [providers, setProviders] = useState<AuthProviders>({ email: true, google: false, github: false });
  const setToken = useSetAtom(tokenWriteAtom);
  const navigate = useNavigate();

  useEffect(() => {
    fetchAuthProviders().then((res) => {
      ...
      <div className="panel p-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-3 mb-6">
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full py-3 btn-primary text-white rounded-xl font-semibold disabled:opacity-50 transition-colors"
          >
            {loading
              ? 'Please wait...'
              : mode === 'register'
        ...
        <div className="space-y-3">
          ...
          {!providers.google && !providers.github && (
            <p className="text-xs text-slate-500 text-center">
              Third-party sign-in is not configured yet.
            </p>
          )}
        </div>
